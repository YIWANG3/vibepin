# vpin — vibepin file-watcher loop (Cursor)
...
burns tokens while idle. This loop blocks in the terminal instead, so no model tokens
...
1. Run this in the terminal and wait for it — it blocks until a new annotation
   arrives, then exits:
   `npx vibepin watch`
